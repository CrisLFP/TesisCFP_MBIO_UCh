...
def_2022_2025 <- read_csv("def_2022_2025.csv") #considerar que estos no están verificados
```
...
ServSalud <- read_csv("ServSalud.csv") # ESTE ARCHIVO SE CREÓ EN SCRITP DE RPC, DEBE COPIARSE Y PEGARSE EN ESTA CARPETA, O BIEN PONER LA DIRECCIÓN DE ESTE ARCHIVO EN SU COMPUTADORA

## crear variable SS EN DEFUNCIONES. SOLO CORRER ESTE CHUNK TAL COMO ESTA (NO QUITAR LOS #s)
ServSalud<- ServSalud %>%
...
```{r message=F, eval = F}
## Reagrupar por mediana, terciles y cuartiles para no tener def = 0 en modelado. SE CONSIDERARON ESTAS ALTERNATIVAS EN RELACIÓN A LAS RECOMENDACIONES DEL MINSAL PARA EL MODELADO DE RIM, EN CASO DE QUE UN QUINQUENIO DE EDAD LAS DEFUNCIONES FUERAN 0 (SE INDEFINE LA RIM)
...
```{r}
## ESTOS ACRHIVOS DE CREAN DESDE EL SCRIP DE PROYECCIONES POBLACIONALES. INSERTE LA RUTA DE TRABAJO DE SU ORDENADOR PARA LLAMAR A CADA UNO DE ELLOS.
